...
#define LIGHT_PIN 4
#define BUTTON_PIN 2
#define ssidMaxChars 50
#define passwordMaxChars 70
...
// Modify this section (add your deployment ID)
// ---------------------------------------------------------------------
// your Google Script Deployment ID 
const char *DeploymentID = "<INPUT ID HERE>";
// you can customize the interval of times at which the lamp will check for a colour change
// it is currently every 5 minutes
...
// portal is active for only 10 minutes
...
// Declaring a struct - used for saving variables in memory
struct wifiConfig{
    byte isSet;
    char ssid[ssidMaxChars];
    char pass[passwordMaxChars];
 } savedConfig;
...
const char* config_html = R"===(<!DOCTYPE HTML><html><head> 
...
void setup() {
  ...
  if (!isWifiConnected){
    ...
    // this sets the name and password for the temporary portal that will appear for 10 minutes
    Serial.println(WiFi.softAP("Companion Cube", "apple_pine_windOw2") ? "Ready" : "Failed!");
  
    Serial.print("Soft-AP IP address = "); 
    Serial.println(WiFi.softAPIP());

    // display screen to client 
    server.on("/", displayPage);
    server.on("/connect", getWifiConfig);
      
    server.onNotFound(handle_NotFound);
    server.getServer().setRSACert(new BearSSL::X509List(serverCert), new   BearSSL::PrivateKey(serverKey));
    server.begin();

    // the screen can be accessed via https://companion-cube.local
...
int first = 1;
void loop() {
  // keep the WiFi Access portal open for 10 minutes then close it
  ...
  if (currentButtonState == LOW) {
    ...
    // user can now release the button/switch
  ...
  ;
    ...
    //read data from google sheet if period of time has passed
    currentMillis = millis();
...
void displayPage(){
  // Function to display the main page to configure the WiFi settings
  server.send(200, "text/html", config_html); 
}


void handle_NotFound(){
  // Function to display when the page the client is looking for is not found
...
void getWifiConfig()
{
  // Function triggered when the user info is submitted on the screen
  ...
  MDNS.update();
  server.handleClient();
  // Add a header to go to the home page again
...
void updateServer(){
  ...
  if(isServerOn){
    // keep the local domain accessible
    MDNS.update();
    // calls the correct callback function from server.on based on the client interactions
    server.handleClient();
  }
}
...
void connectWifi(){
  // Function to establish a WiFi connection using the user input or the data saved in memory
   if (wifiSSID == "" || pass == ""){
      return;
   }
   updateServer();
  ...
  if (WiFi.status() == WL_CONNECTED){
    ...
    if(isServerOn){
      ...
      wifiSSID.toCharArray(savedConfig.ssid, ssidMaxChars);
      pass.toCharArray(savedConfig.pass, passwordMaxChars);
      savedConfig.isSet = 0;
...
bool initialConnectionToGoogleSheets(){
  // Function to check if a connection to Google Sheets can be established
...
void reconnectWifi(){
  // Function to make sure the WIFI connection has not been lost
  WiFi.begin(wifiSSID, pass); 
